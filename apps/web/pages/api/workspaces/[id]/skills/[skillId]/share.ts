...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const skillId = Array.isArray(req.query.skillId) ? req.query.skillId[0] : req.query.skillId;
  if (!id) {
  ...
  const backend = await fetch(
    `${apiUrl()}/workspaces/${encodeURIComponent(id)}/skills/${encodeURIComponent(skillId)}/share`,
    {
      method: 'POST',

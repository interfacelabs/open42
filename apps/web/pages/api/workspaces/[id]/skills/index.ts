...
/**
 * Skills list (GET) + mint (POST) proxy. Forwards to the backend
 * `/workspaces/:id/skills` route. The workspace id rides in the URL so the
 ...
 *
 * Body shape on POST: `{ intent: string; threadCitations: Array<...> }`.
 */
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const backend = await fetch(
    `${apiUrl()}/workspaces/${encodeURIComponent(id)}/skills`,
    init,
  );
